...
class Bizzio_Sync_Gencloud_Admin
{
	...
	{
		echo '<input id="' . $arguments['uid'] . '" name="' . $arguments['uid'] . '" type="' . $arguments['type'] . '" placeholder="' . $arguments['placeholder'] . '" value="' . get_option($arguments['uid']) . '" />';
		if ($arguments['helper']) {
			echo '<span class="helper">' . $arguments['helper'] . '</span>';
		}
	}
	...
	public function import_products_callback()
	{
		if (defined('BIZZIO_SYNC_GENCLOUD_DEBUG_LOG') && BIZZIO_SYNC_GENCLOUD_DEBUG_LOG) {
			error_log('import_products_callback hit. Checking nonce: bizzio_import_products_nonce');
		}
	...
	{
		if (defined('BIZZIO_SYNC_GENCLOUD_DEBUG_LOG') && BIZZIO_SYNC_GENCLOUD_DEBUG_LOG) {
			error_log('process_product_batch_callback hit. Checking nonce: bizzio_import_products_nonce');
		}
		check_ajax_referer('bizzio_import_products_nonce', 'security');
		...
		for ($i = 0; $i < $batch_size && $current_progress < $total_articles; $i++) {
			...
			if (! is_wp_error($new_product_id)) {
				...
				$failed_count++;
				if (defined('BIZZIO_SYNC_GENCLOUD_DEBUG_LOG') && BIZZIO_SYNC_GENCLOUD_DEBUG_LOG) {
					error_log('Bizzio Sync Gencloud - Failed to create/update product ' . $product_name . ': ' . (is_wp_error($new_product_id) ? $new_product_id->get_error_message() : 'Unknown error'));
				}
			}
			$current_progress++;
	...
	{
		if (defined('BIZZIO_SYNC_GENCLOUD_DEBUG_LOG') && BIZZIO_SYNC_GENCLOUD_DEBUG_LOG) {
			error_log('get_import_progress_callback hit. Checking nonce: bizzio_import_products_nonce');
		}
	...
	{
		if (defined('BIZZIO_SYNC_GENCLOUD_DEBUG_LOG') && BIZZIO_SYNC_GENCLOUD_DEBUG_LOG) {
			error_log('import_categories_callback hit. Checking nonce: bizzio_import_categories_nonce');
		}
	...
	{
		if (defined('BIZZIO_SYNC_GENCLOUD_DEBUG_LOG') && BIZZIO_SYNC_GENCLOUD_DEBUG_LOG) {
			error_log('process_category_batch_callback hit. Checking nonce: bizzio_import_categories_nonce');
		}
		check_ajax_referer('bizzio_import_categories_nonce', 'security');
		...
		for ($i = 0; $i < $batch_size && $current_progress < $total_categories; $i++) {
			...
			if (! is_wp_error($term_id_result)) {
				...
				$failed_count++;
				if (defined('BIZZIO_SYNC_GENCLOUD_DEBUG_LOG') && BIZZIO_SYNC_GENCLOUD_DEBUG_LOG) {
					error_log('Bizzio Sync Gencloud - Failed to create/update category ' . $category_name . ': ' . $term_id_result->get_error_message());
				}
			}
			$current_progress++;
	...
	{
		if (defined('BIZZIO_SYNC_GENCLOUD_DEBUG_LOG') && BIZZIO_SYNC_GENCLOUD_DEBUG_LOG) {
			error_log('get_category_import_progress_callback hit. Checking nonce: bizzio_import_categories_nonce');
		}
		check_ajax_referer('bizzio_import_categories_nonce', 'security');
	...
	private function _make_soap_request($soap_action, $request_body)
	{
		...
		if (defined('BIZZIO_SYNC_GENCLOUD_DEBUG_LOG') && BIZZIO_SYNC_GENCLOUD_DEBUG_LOG) {
			error_log('Bizzio Sync Gencloud - Response: ' . print_r($response, true));
		}
	...
	private function _set_woocommerce_category_thumbnail($term_id, $image_url)
	{
		...
		if (empty($image_url) || !filter_var($image_url, FILTER_VALIDATE_URL)) {
			if (defined('BIZZIO_SYNC_GENCLOUD_DEBUG_LOG') && BIZZIO_SYNC_GENCLOUD_DEBUG_LOG) {
				error_log('Bizzio Sync Gencloud - Invalid image URL for term ID ' . $term_id . ': ' . $image_url);
			}
			return;
		}
		...
		if (is_wp_error($tmp)) {
			if (defined('BIZZIO_SYNC_GENCLOUD_DEBUG_LOG') && BIZZIO_SYNC_GENCLOUD_DEBUG_LOG) {
				error_log('Bizzio Sync Gencloud - Failed to download image for term ID ' . $term_id . ': ' . $tmp->get_error_message());
			}
			return;
		}
		...
		if (! is_wp_error($attach_id)) {
			...
			if (defined('BIZZIO_SYNC_GENCLOUD_DEBUG_LOG') && BIZZIO_SYNC_GENCLOUD_DEBUG_LOG) {
				error_log('Bizzio Sync Gencloud - Failed to set category thumbnail for term ID ' . $term_id . ': ' . $attach_id->get_error_message());
			}
		}
	}
	...
	private function _set_woocommerce_product_image($product_id, $image_url)
	{
		...
		if (empty($image_url) || !filter_var($image_url, FILTER_VALIDATE_URL)) {
			if (defined('BIZZIO_SYNC_GENCLOUD_DEBUG_LOG') && BIZZIO_SYNC_GENCLOUD_DEBUG_LOG) {
				error_log('Bizzio Sync Gencloud - Invalid image URL for product ID ' . $product_id . ': ' . $image_url);
			}
			return;
		}
		...
		if (is_wp_error($tmp)) {
			if (defined('BIZZIO_SYNC_GENCLOUD_DEBUG_LOG') && BIZZIO_SYNC_GENCLOUD_DEBUG_LOG) {
				error_log('Bizzio Sync Gencloud - Failed to download image for product ID ' . $product_id . ': ' . $tmp->get_error_message());
			}
			return;
		}
		...
		if (! is_wp_error($attach_id)) {
			...
			if (defined('BIZZIO_SYNC_GENCLOUD_DEBUG_LOG') && BIZZIO_SYNC_GENCLOUD_DEBUG_LOG) {
				error_log('Bizzio Sync Gencloud - Failed to set product image for product ID ' . $product_id . ': ' . $attach_id->get_error_message());
			}
		}
	}
